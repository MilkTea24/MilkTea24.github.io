...
B-Tree에서 실제 데이터의 주소값을 직접 참조할 때는 비클러스터링 인덱스라고 한다.
MyISAM 엔진은 비클러스터링 인덱스를 채택하고 있다.

반면 실제 데이터의 주소값을 직접 참조하지 않고 프라이머리 키 인덱스를 통해 한번 더 인덱스 탐색을 거치는 경우 클러스터링 인덱스라고 한다.
InnoDB 엔진은 클러스터링 인덱스를 채택하고 있다.
클러스터링 인덱스는 두 번 탐색하므로 성능이 떨어지지만 InnoDB에서 이를 채택하고 있는 이유가 있는데 이는 클러스터링 인덱스에서 자세히 알아볼 예정이다.
