...
| student_fk | student_id | subject_id |
|------------|------------|------------|
| 1          | 202103242  | 1          |
| 1          | 202103242  | 3          |
| 2          | 201903215  | 2          |
...
위와 같이 학생-성적 테이블을 저장한다면 학생의 학번만 필요한 경우 학생 테이블을 조인하지 않고 학생-성적 테이블과 성적 테이블을 조회하면 학번에 해당하는 성적을 알 수 있다.
학생이 이름이 필요한 경우 그 때만 학생과 학생-성적 테이블을 조인하면 된다.

하지만 이렇게 저장하게 된다면 데이터가 중복이 되어 데이터의 일관성이 깨질 수 있다.
학번이 바뀌는 경우 학생 테이블과 학생-성적 테이블 모두 학번을 변경해야 할 것이다.
...
따라서 수직 분할 이전에 비해 적은 수의 페이지만 불러오므로 성능이 향상된다.
