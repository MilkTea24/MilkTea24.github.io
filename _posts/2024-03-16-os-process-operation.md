...
3. 자식 프로세스는 작업을 수행한다.
   ...
   - 또는 exec System Call을 이용하여 자식 프로세스가 수행할 프로그램을 지정할 수 있다.
...
execlp() System Call은 실행되면 부모 프로세스에서 물려받은 데이터를 새로운 프로그램의 데이터로 완전히 교체한다.
...
## Windows 자식 프로세스 생성
Windows는 fork와 CreateProcess 두 가지 System Call을 가진다.
fork는 부모 프로세스부터 주소 공간을 상속받아 자식 프로세스를 생성한다.
CreateProcess는 자식 프로세스가 생성될 때 새로운 주소 공간을 생성한다.
...
하지만 부모 프로세스가 이를 회수하지 않으면 커널은 자식이 종료되더라도 이 자식 프로세스의 최소한의 정보를 가지고 있다.
이 최소한의 정보를 좀비 프로세스라 하며 이는 메모리 사용량이 증가하는 요소가 된다.
