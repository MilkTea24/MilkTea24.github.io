...
데드락은 서로가 필요한 자원을 락으로 점유하고 있어 더 이상 트랜잭션을 진행하지 못하고 서로 교착 상태에 빠지게 되는 상태를 말한다.
...
운영체제의 락과 비슷하게 DB의 락 또한 상호 배제, 비선점 특성을 가진다.
이 상황에서 트랜잭션들이 점유 대기, 순환 대기 조건을 만족하면 데드락이 발생하게 된다.

1. 배타적 락은 다른 트랜잭션이 배타적 락과 공유 락을 획득하는 것을 막아 상호 배제적 특성을 가진다.
2. 트랜잭션 내에서 여러 자원에 대한 잠금 획득을 요청할 수 있으므로 하나의 데이터에 대한 잠금을 가지면서 다른 데이터에 대한 잠금을 얻기 위해 기다리는 점유 대기 상황이 발생할 수 있다.
3. 배타적 락은 획득한 트랜잭션이 직접 해제하기 전까지 다른 트랜잭션이 이 락을 강제로 해제하여 자원에 접근할 수 없는 비선점 특성을 가진다.
4. 점유 대기 관계가 사이클을 형성하는 순환 대기 상황이 발생할 수 있다.

따라서 DB의 락 또한 데드락이 발생할 가능성이 있으므로 데이터베이스 관리 시스템은 데드락을 감지하여 처리할 수 있어야 한다.

## MySQL의 데드락 해결 방법

InnoDB 스토리지 엔진은 데드락에 빠지지 않았는지 체크하기 위해 잠금 대기 목록을 그래프 형태로 관리한다.
이 역할은 InnoDB 스토리지 엔진의 데드락 감지 스레드가 주기적으로 잠금 대기 그래프를 검사하여 사이클이 있는지 확인한다.

데드락이 발생된 상태로 확인되면 교착 상태에 빠진 트랜잭션들 중 하나를 종료한다.
이 때 언두 로그 레코드를 더 적게 가진 트랜잭션이 일반적으로 롤백의 대상이 된다.

### innodb_deadlock_detect

데드락 감지 스레드를 활성화/비활성화 할 수 있는 시스템 변수이다.
트랜잭션의 수가 많은 경우 잠금의 개수가 많아져 데드락 감지 스레드가 느려지게 된다.
데드락 감지 스레드는 **검사 도중 잠금 상태가 변경되지 않도록 잠금 대기 목록을 저장한 테이블에 새로운 잠금을 걸고 찾는다.**
그러므로 데드락 감지 스레드로 인해 작업 진행 속도가 느려지고 서비스에 악영향을 미칠 수 있다.
이 경우네는 데드락 감지 스레드를 비활성화하여 성능을 향상시킬 수 있다.

### innodb_lock_wait_timeout

데드락 감지 스레드가 비활성화된 경우 데드락이 발생하면 무한정 대기하게 될 위험이 있다.
따라서 InnoDB는 락의 획득 시간이 일정 기준선을 초과하면 자동으로 요청이 실패하고 에러 메세지를 반환하게 할 수 있다.
